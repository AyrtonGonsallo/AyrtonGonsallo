...
def chunk_search(segment, chunked):
    ...
    list1 = []
    for j in range(m):
        if (len(chunked[j]) > 2 or len(chunked[j]) == 1):  # chunked[j] est de la forme ('Harold', 'NNP')
            list1.append(j)
        if (len(chunked[j]) == 2):
            try:
                str1 = chunked[j][0][0] + " " + chunked[j][1][0]  # premier caractere de chaque tuple ('Harold', 'NNP') =>H N
            except Exception:
                pass
            else:
